...
class ImageInput extends Component {
	...
	async handleFileChange(file) {
		...
		if (!file) {
			...
			this.setState({message: imageTooLargeMessage});
		} else {
	...
	async upload(image) {
		...
		if (response.error) {
			return this.setState({
				isLoading: false,
				message: 'There was a problem with the image upload, please try again or select a different image.',
			});
		}
	...
	render() {
		...
		const { isLoading, isDragging, message } = this.state;
